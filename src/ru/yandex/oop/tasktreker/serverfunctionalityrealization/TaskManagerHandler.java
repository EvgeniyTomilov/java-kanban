...
import static ru.yandex.oop.tasktreker.model.enums.TaskType.*;
    ...
    public class TaskManagerHandler implements HttpHandler {
        ...
        private void handlePostOrUpdateEpic(HttpExchange exchange) throws IOException {
            System.out.println("inside handle post");
            String body = new String(exchange.getRequestBody().readAllBytes(), DEFAULT_CHARSET);
            try {
                ...
                if (newEpicTask.getId() != 0 && manager.getAnyTask(newEpicTask.getId()) != null) {
                    ...
                    System.out.println("id is " + (manager.createTaskAndReturnId(newEpicTask)));
        ...
        private Task deserializeTask(JsonElement jsonElement){
            System.out.println("inside deserialize 1");
            ...
            System.out.println("starttime is " + jsonObject.get("StartTime").getAsString());
            ...
            System.out.println("inside deserialize 2");
            task.setId(id);
            System.out.println("inside deserialize 1");
            return task;
        }
        ...
        private SubTask deserializeSubtask(JsonElement jsonElement){
            ...
            System.out.println("reached 1");
            String description = jsonObject.get("Description").getAsString();
            System.out.println("EpidId is " + jsonObject.get("EpicId"));
            int epicId = jsonObject.get("EpicId") == null ? 0 : jsonObject.get("EpicId").getAsInt();
            System.out.println("reached 1");
            Duration duration = Duration.ofMinutes(jsonObject.get("Duration").getAsLong());
        ...
        private EpicTask deserializeEpic(JsonElement jsonElement){
            ...
            System.out.println("inside deserialize 2");
            int id = jsonObject.get("Id").getAsInt();
            System.out.println("inside deserialize 3");
            String name = jsonObject.get("name").getAsString();
            System.out.println("inside deserialize 4");
            String description = jsonObject.get("Description").getAsString();
            System.out.println("inside deserialize 5");;
